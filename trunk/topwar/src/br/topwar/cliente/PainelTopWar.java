...
public class PainelTopWar {
	...
	private void desenhaControleMudarClasse(Graphics2D graphics2d) {
		...
		x += 110;
		shieldRect.setFrame(x, y, lifeBarShield.getWidth(), lifeBarShield
				.getHeight());
	...
	private void gerarMinis() {
		...
		lifeBarShield = ImageUtil.gerarFade(ImageUtil.geraResize(riot_shield,
				0.75, 0.25), 255);
	...
	private void geraPainel() {
		panel = new JPanel() {
			public Dimension getPreferredSize() {
				return new Dimension(mapaTopWar.getLargura(), mapaTopWar
						.getAltura());
			}
			...
			private void desenhaChat(Graphics2D graphics2d) {
				...
				for (int i = 0; i < radioMsgCopiaPainel.size(); i++) {
					...
					graphics2d.fillRoundRect(x - 5, nvY - 13, Util
							.calculaLarguraText(lnTxt + "   ", graphics2d), 16,
							5, 5);
	...
	protected void desenhaBackGround(Graphics2D graphics2d) {
		if (backGround == null) {
			carregaBackGround();
		} else {
				...
				Runnable runnable = new Runnable() {
					...
					public void run() {
						backGround = ImageUtil.carregaImagemWebContent(
								mapaTopWar.getBackGround(), jogoCliente
										.getApplet(), "br/topwar/recursos/");
						if (backGround != null) {
	...
	private void desenhaPlacar(Graphics2D g2d) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			g2d
					.drawString("" + placarTopWar.getClasse(), xJogador + 110,
							yTemp);

			g2d.setColor(ConstantesTopWar.lightBlu);
			g2d.fillRoundRect(xKills - 5, yTemp - 15, Util.calculaLarguraText(
					Lang.msg("000"), g2d) + 10, 20, 10, 10);
			g2d.setColor(Color.BLACK);
	...
	private void desenhaInfoBaixo(Graphics2D g2d) {
		...
		if (jogoCliente.verificaRecarregando()) {
			...
			g2d.fillRoundRect(x - 160, y - 30, Util.calculaLarguraText(Lang
	...
	private void desenhaCampoVisao(Graphics2D graphics2d,
		...
		Point back = GeoUtil.calculaPonto(avatarCliente.getAngulo() + 180, 30,
		...
		BufferedImage bufferedImage = new BufferedImage(limitesViewPort
				.getBounds().width, limitesViewPort.getBounds().height,
				BufferedImage.TYPE_INT_ARGB);
